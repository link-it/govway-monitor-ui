 ...
 *     Object.extend(target, source)   -> Object.assign(target, source)
 *     Element.show(id) / Element.hide -> el.style.display = '' / 'none'
 *     Event.observe / stopObserving   -> addEventListener / removeEventListener
 *     bindAsEventListener(this)       -> .bind(this)
 *   ModalPanel.Border, ModalPanel.Sizer, ModalPanel.Sizer.Diff sono attaccati
 *   sul global ModalPanel definito in modalPanel.js (caricato prima nel <h:scripts>).
 * Copyright (c) 2022-2026 Link.it srl (https://link.it).
 *
 * Distribuito sotto la stessa licenza LGPL v2.1 di RichFaces 3.3.4.Final.
 */

ModalPanel.Border = function (id, modalPanel, cursor, sizer) {
	this.initialize(id, modalPanel, cursor, sizer);
};
...
ModalPanel.Border.prototype = {
	initialize: function(id, modalPanel, cursor, sizer) {
		...
		this.boundStartDrag = this.startDrag.bind(this, new Date());
		element.addEventListener('mousedown', this.boundStartDrag);
	...
	destroy: function()
	{
		...
		if (element) {
			element.removeEventListener('mousedown', this.boundStartDrag);
		}
	...
	startDrag: function(event) {
		...
		var eCursorDiv = document.getElementById(this.modalPanel.cursorDiv);
		eCursorDiv.style.cursor = document.getElementById(this.id).style.cursor;
	...
	doDrag: function(event) {
		...
		if (dx != 0 || dy != 0) {
			...
			var element = document.getElementById(this.modalPanel.cdiv);
	...
	endDrag: function(event) {
		...
		document.getElementById(this.modalPanel.cursorDiv).style.zIndex = -200;
	...
	doPosition: function() {
		this.sizer.doPosition(this.modalPanel, document.getElementById(this.id));
	}
};

ModalPanel.Sizer = function () {};

ModalPanel.Sizer.INITIAL_MIN = 4;
ModalPanel.Sizer.INITIAL_MAX = 40;

ModalPanel.Sizer.Diff = function (dX, dY, dWidth, dHeight) {
	this.initialize(dX, dY, dWidth, dHeight);
};
...
ModalPanel.Sizer.NWU = Object.assign(new ModalPanel.Sizer(), {
	doPosition: function (modalPanel, elt) {

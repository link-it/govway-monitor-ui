...
public abstract class UIComponentBase extends UIComponent {
    ...
    private static boolean isSaveBindingsState(FacesContext context) {
        ...
        if (null == (flag = (Boolean)requestMap.get(IS_SAVE_BINDINGS_STATE_PARAM_NAME))) {
            Map<String, Object> appMap = extContext.getApplicationMap();
            if (null == (flag = (Boolean) appMap.get(IS_SAVE_BINDINGS_STATE_PARAM_NAME))) {
                String setting = extContext.getInitParameter(IS_SAVE_BINDINGS_STATE_PARAM_NAME);
                flag = (setting == null || Boolean.parseBoolean(setting));
                appMap.put(IS_SAVE_BINDINGS_STATE_PARAM_NAME, flag);
            }
            requestMap.put(IS_SAVE_BINDINGS_STATE_PARAM_NAME, flag);
        }
    ...
    public Object saveState(FacesContext context) {
        ...
        Object bindingsState = saveBindingsState(context);
        if (isSaveBindingsState(context)) {
            values[1] = bindingsState;
        }
        values[2] = clientId;
        values[3] = id;

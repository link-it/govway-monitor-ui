 ...
 * 
 * @author Jacob Hookom
 * @version $Id: ComponentHandler.java,v 1.19.2.1 2009/08/19 21:43:58 edburns Exp $
 */
public class ComponentHandler extends MetaTagHandler {
    ...
    /**
     * <p>If this parameter exists and calling Boolean.parseBoolean() on
     * its value returns true, any EL expressions present on the
     * component markup will be re-applied to the UIComponent instance
     * from markup, rather than assuming they will have been included in
     * the saved state for the UIComponent instance.</p>
     */ 
    ...
    public final void apply(FaceletContext ctx, UIComponent parent)
	    ...
	    // HPD: here we could re-create the ValueExpressions
	    if (isRecreateValueExpressionsOnBuildBeforeRestore(ctx.getFacesContext())) {
		recreateValueExpressions(ctx, c);
	    }

        } else {
	    // HPD: component is new
  ...
  private boolean isRecreateValueExpressionsOnBuildBeforeRestore(FacesContext context) {
    ...
    if (null == (flag = (Boolean)appMap
        .get(PARAM_RECREATE_VALUE_EXPRESSION_ON_BUILD_BEFORE_RESTORE))) {
      String setting = extContext
          .getInitParameter(PARAM_RECREATE_VALUE_EXPRESSION_ON_BUILD_BEFORE_RESTORE);
      flag = (setting == null || Boolean.parseBoolean(setting)) ? Boolean.TRUE : Boolean.FALSE;
      if (flag.booleanValue()
          && (!"true".equals(extContext
              .getInitParameter(FaceletViewHandler.PARAM_BUILD_BEFORE_RESTORE)) && !"true"
              .equals(appMap.get(FaceletViewHandler.PARAM_BUILD_BEFORE_RESTORE)))) {
        flag = Boolean.FALSE;
        log.warning(PARAM_RECREATE_VALUE_EXPRESSION_ON_BUILD_BEFORE_RESTORE
            + " is set to 'true' but " + FaceletViewHandler.PARAM_BUILD_BEFORE_RESTORE
            + " is set to 'false' or unset. To use "
            + PARAM_RECREATE_VALUE_EXPRESSION_ON_BUILD_BEFORE_RESTORE + " you must also set "
            + FaceletViewHandler.PARAM_BUILD_BEFORE_RESTORE + " to 'true'!");
      }
